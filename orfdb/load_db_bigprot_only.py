...
from orfdb import base, settings, annotation_loading, settings
...
def load_db_logic(drop_all: bool = False) -> None:
    """Main logic for loading the ORF database.
...
@click.command()
@click.option('--drop-all', is_flag=True, help='Empty database and reload data.', default=False)
def load_db(drop_all: bool) -> None:
    ...
    Args:
        drop_all: If True, drops and recreates all database tables.
    """
    load_db_logic(drop_all=drop_all)
